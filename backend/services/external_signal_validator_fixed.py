...
class ExternalSignalPerformanceValidator:
    ...
    async def track_external_signal_execution(
        ...
        try:
            ...
            if result.data and len(result.data) > 0:
                ...
                update_data = {
                    "total_signals_received": record.get("total_signals_received", 0) + 1,
                    "total_signals_executed": record.get("total_signals_executed", 0) + (1 if executed else 0),
                    "signals_ignored": record.get("signals_ignored", 0) + (1 if not executed else 0),
    ...
    async def record_external_signal_closure(
        ...
        try:
            ...
            if result.data and len(result.data) > 0:
                ...
                update_data = {
                    "executed_win_count": new_win_count,
                    "executed_loss_count": new_loss_count,
                    "executed_win_rate": new_win_rate,
                    "total_pnl": new_total_pnl,
                    "total_roi_pct": new_total_pnl * 100,
                    "avg_roi_per_trade": (new_total_pnl / (exec_count + 1)) * 100 if exec_count >= 0 else 0,
                ...
                # Create new record for first trade
                reliability_score = self._calculate_reliability_score(1.0 if is_win else 0.0, 0, pnl, pnl)
                recommendation = self._get_reliability_recommendation(reliability_score, 1)
                ...
                insert_data = {
                    ...
                    "period_start_date": str(period_date),
                    "period_end_date": str(period_date),
                    "executed_win_count": 1 if is_win else 0,
                    "executed_loss_count": 0 if is_win else 1,
                    "executed_win_rate": 1.0 if is_win else 0.0,
                    "total_pnl": pnl,
                    "total_roi_pct": pnl * 100,
                    "avg_roi_per_trade": pnl * 100,
                    "reliability_score": reliability_score,
                    "recommendation": recommendation,
                ...
                logger.info(f"✅ Created external signal closure record: {source}")
                return {
                    "success": True,
    ...
    async def get_source_reputation(
        ...
        try:
            ...
            result = self.db.supabase.table("external_signal_performance")\
                ...
                .eq("user_id", user_id)\
                .eq("external_source", source.value)\
                .gte("period_end_date", str(period_start))\
                .order("period_end_date", desc=True)\
                .limit(1)\
                .execute()
            
            if result.data and len(result.data) > 0:
                logger.info(f"📊 Retrieved reputation for {source}: {result.data[0].get('recommendation', 'N/A')}")
    ...
    async def get_all_source_rankings(
        ...
        try:
            results = self.db.supabase.table("external_signal_performance")\
                .select("*")\
                .eq("user_id", user_id)\
            ...
            logger.info(f"✅ Retrieved {len(results.data)} source rankings for user {user_id}")
